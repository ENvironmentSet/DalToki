...
var Runner = ( () => {
    ...
    class Func {
        ...
        static tokensToArgument (tokens,scope) {
            let result = tokens.map( function saveOnlyValues (v) {
                let item;
                if(!(v.type === "identifier" && (item = scope[v.value]) !== undefined)) {
                    item = new Var(v.value,v.type);
                }
                return item;
            });
            return result.filter(function removeUndefined (v) {
    ...
    class Var { // 참조 관련 작업 필요.
        constructor (value,type) {
            ...
            this.assignment = function assignment (_item,type) {
                if (_item.constructor === Var) return this.refernce(_item);
                else return this.change(_item,type);
            };
