...
var Runner = ( () => {
    ...
    class _Object extends Value {
        constructor (values,identifiers) {
            super("Object",_Object.tokenToObject(values,identifiers) || {});
        }
        static tokenToObject(value_tokens,identifier_tokens) {

        }
        static unwrapALLvalues(object) {

        }
    }

    class Class {
        constructor (Create,Static,Public,Private) {
        ...
        switch (tree.type) {
            ...
            case "FUNC":
                if(tree.hasAttrs(["NAME","ARGS"])) { // 나중에 new Var로 변경
                    let name = tree.options.get("NAME").value;
            ...
            case "VAR":
                if(tree.hasAttrs(["NAME"])) { // 나중에 new Var로 변경
                    ...
                    } else variable = new Var(value,type !== undefined ? type[0].value.toLowerCase(): type);
            ...
            case "ARRAY":
                if(tree.hasAttrs(["NAME"])) { // 나중에 new Var로 변경
                    ...
                    let array = new _Array(values);
                    tree.addValue(tree.options.get("NAME").shift().value, array);
                } else throw new RunnerException("VAR container require Attr [NAME]");
                break;
            default :
