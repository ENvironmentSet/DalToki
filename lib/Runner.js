    ...
    class _Array extends Value {
        ...
        static tokekToLinkedList (tokens) {
            ...
            for (let token of tokens) {
                if(result === null) result = new _Element(token.value,token.type);
                else result.attach(new _Element(token.value,token.type));
            }
            return result;
        }
    }
        ...
        switch (tree.type) {
            ...
            case "ARRAY":
                if(tree.hasAttrs(["NAME"])) {
                    ...
                    let name = tree.options.get("NAME")[0].value;
                    let array = new _Array(_Array.tokekToLinkedList(values));
