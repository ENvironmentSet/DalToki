        ...
        switch (tree.type) {
            ...
            case "VAR":
                if(tree.hasAttrs(["NAME"])) { // 나중에 new Var로 변경
                    ...
                    let variable;
                    if(type) {
                        variable = Var.define(type.shift().value,value ? value.shift().value : undefined);
                    } else variable = Var.define_type_inference(value ? value.shift().value : undefined);
            ...
            case "ARRAY":
                ...
                break;
            default :
                let target = tree.scope.__proto__[tree.type];
        }
    }
