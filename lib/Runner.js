...
var Runner = ( () => {
    ...
    class _Array extends Value {
        ...
        static tokekToLinkedList (tokens) {
            ...
            for (let token of tokens) {
                if(result === null) context = result = new _Element(new Var(token.value,token.type));
                else context = context.attach(new _Element(new Var(token.value,token.type)));
            }
            return result;
        }

        push (variable) {
            var target = this.index(this.length-1);
            target.append(new _Element(variable));
            return this.length += 1;
        }

        pop () {
            var target = this.index(this.length-2);
            target.remove();
            return this.length -= 1;
        }
        ...
        shift () {
            ...
            this.value = target.getNext();
            return this.length -= 1;
        }

        unshift (variable) {
            var target = this.value;
            this.value = new _Element(variable);
            this.value.attach(target);
            return this.length += 1;
        }
    ...
    class Func {
        ...
        static initScope (func) {
            var iterator = Object.keys(func.scope);
            while(iterator.length) {
                func.scope[iterator.shift()] = {"type":"VOID","value":null};
            }
        }
        ...
        callFunc (args,_scope,_callback) {
            ...
            switch (this.func_type) {
                case "NORMAL":
                    ...
                    break;
                case "BUILT-IN":
                    args.push(Object.create(_scope),_callback);
    ...
    class Var { // 배열 관련 작업 필요
        ...
        static define_type_inference (value) {
            ...
            if(type === String) {
                ...
                return new _Array(Object.assign({},value));
            } else if (type === Array) {
