var parser = function parser (tokens) {
    ...
    /*class Expression { // < | >
        constructor (option) { // Refernce [ RelOp Refernce ]
            var t = option[0];
            if(t === undefined) throw new ParserException("No token For Formula parsing");
            Expression.check(t);
            this.left = option.shift();
            t = option[0];
            if(t !== undefined&& t.type === "operator") {
                this.type = option.shift();
                t = option[0];
                this.right = new Expression(option);
            }
        }

        static check (t) {
            if(t.type !== "integer" && t.type !== "float" && t.type !== "string" && t.type !== "identifier") throw new ParserException("only integer,float,string,identifier can't be left-hand");
        }

        reslove (scope) {
            var result = this.left.value;
            var cache;
            if(this.operator) {
                switch (this.operator.value) {
                    case "-":
                        cache = this.right.reslove(scope);
                        if(cache.type === "identifier") cache = scope[cache.value];
                        result -= cache;
                        break;
                    case "+":
                        cache = this.right.reslove(scope);
                    ...
                    case ".":
                        break;
                    default :
                        throw new ParserException("Unknown operator",this.operator);
                }
            }
            log(result)
            return result;
